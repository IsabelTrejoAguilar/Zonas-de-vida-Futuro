...
#####Climate Data####
# Climate scenarios
setwd("C:/Users/Angela/Downloads/Delta_MPI_ESM_LR_rcp45_2015-2039_tmed/")
capas<-stack(list.files("GeoTIFF",pattern="*.tif$",full.names = TRUE))
at_delta<-mean(capas)
plot(at_delta, main = "MPI_ESM_LR_rcp45_2015_2039")
writeRaster(at_delta, filename="ano_tmean.tif", format="GTiff", overwrite = TRUE)
#at_delta<- raster("C:/CONABIO/CLIMA/UNIATMOS_anos/MPI_ESM_LR_rcp45_2015_2039_bio/ano_tmean.tif") ##Raster temperature (?C)
ap<- raster("C:/Proyectos/PNUD_ISLAS/Deltas/MPI_ESM_LR_rcp45_2015_2039_bio/_bio12/w001001.adf") ##Raster precipitation (mm)
ap
alt <-raster("C:/CONABIO/CLIMA/LifeZones/LZ_R_uniatmos/alt//w001001.adf")
alt

#Current climate
at_present <- raster("F:/COBERTURAS/UNIATMOS_CCA/Presente/bio1.asc") ##Raster temperature (°C)
at_present
plot(at_present)
summary(at_present)

## Project Raster
at_present <- projectRaster(at_present,ap,crs = WGS84, method="ngb")
at_delta <- projectRaster(at_delta, at_present, crs = WGS84, method="ngb")

extent(at_present) <- extent(ap)
extent(at_present) <- extent(at_delta)

##Verify if data and extent are the same ##Answer must be TRUE

compareRaster(at_present, ap, at_delta, alt)
#Set working directoy
setwd("C:/CONABIO/CLIMA/LifeZones/LZ_R_uniatmos")
dir.create("MPI_ESM_LR_rcp45_2015_2039_bio")
setwd("C:/CONABIO/CLIMA/LifeZones/LZ_R_uniatmos/MPI_ESM_LR_rcp45_2015_2039_bio") #Change directory to work

##Save the new files

writeRaster(at_present, filename="at_present.tif", format="GTiff", overwrite=TRUE)
writeRaster(ap, filename="ap.tif", format="GTiff", overwrite=TRUE)
...
#####Biotemperature (present)####
##Calculate biotemperature with temperature of the present and latitude in grade

biotTem <- at_present ##Create a new file from the original
...
biot_present <- merge(bioupto24,bio0to24)
plot(biot_present, main = "Present Biotempertura")
#writeRaster(biot_present, filename="biot_present.tif", format="GTiff", overwrite=TRUE)
#biot<-biot_present
rm(bio0to24, bio0to30, bio24to30, biotTem, bioupto24, coor, x, x_lat)

####Future####
##Biotemperature
suma<-biot_present+at_delta
plot(suma, main = "Suma")
biotTem <-suma
summary(biotTem)
...
#plot(biotTem)
bio0to30 <- biotTem ##Create a file with all the classified data
plot(bio0to30, main = "Bio0to30 Future")

##The data up to 24.0 need a correction: biot = temperature - [3 * latitudegrade/100) * (temperature - 24)^2
is.na (biotTem) <- biotTem >= 24.1  
bio0to24 <- biotTem #areas with less than 24°C
plot(bio0to24, main = "Bio0to24 Future")
biotTem <- bio0to30
plot(biotTem)
is.na (biotTem) <- biotTem <= 23.9
bio24to30 <- biotTem #areas with more than 24°C
plot(bio24to30, main = "Bio24to30 Future")
bioupto24 <- (bio24to30 - (((3.0 * lat) / 100) * ((bio24to30 - 24.0)^2)))
plot(bioupto24, main = "Bioupto24 Future") #biotemp of area with more than 24°C
...
pet <- biot * 58.93
plot(pet)
writeRaster(pet, filename="pet_future.tif", format="GTiff", overwrite=TRUE)

##Calculate potencial evapotranspiration ratio
per <- pet/ap
plot(per)
writeRaster(per, filename="per_future.tif", format="GTiff", overwrite=TRUE)
...
writeRaster(apCat, filename="apCat_future.tif", format="GTiff", overwrite=TRUE)
...
writeRaster(latr, filename="latr_future.tif", format="GTiff", overwrite=TRUE)
...
writeRaster(provhum, filename="provhum_future.tif", format="GTiff", overwrite=TRUE)
...
raster1 <- raster("latr_future.tif") 
raster2 <- raster("apCat_future.tif")
...
raster3 <- raster("provHum_future.tif")
...
writeRaster(raster1, filename="zvh_future.tif", format="GTiff", overwrite = TRUE)
plot(raster1)
